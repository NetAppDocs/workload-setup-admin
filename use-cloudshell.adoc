...
.Diagram of CloudShell tab menu options:
This diagram shows the available menu options when you select the three-dot menu of a open CloudShell tab. The options include rename, duplicate, close other tabs, and close all.
+
...
NOTE: CloudShell is available for any AWS or ONTAP CLI command. However, if you want to work with FSx for ONTAP file systems, make sure issue the following command to work within a specific file system: `using fsx <file-system-name>`.
