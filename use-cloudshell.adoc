...
NetApp CloudShell is a GUI interface built into BlueXP workload factory that allows you to execute AWS CLI commands or ONTAP CLI commands in a shell-like environment. It simulates terminal sessions in the browser, providing terminal features and proxying messages through the backend in workload factory. It allows you to use the AWS credentials and ONTAP credentials that you have provided in your BlueXP Account.
...
+ 
ONTAP commands can be specified with an optional `fsxId`. If an `fsxId` is provided with an individual ONTAP command, this ID will override the ID in the context. If the terminal session doesn't have an FSx for ONTAP file system ID context, then `fsxId` must be passed with each ontap command.
+ 
...
Variables:::
The following are examples of setting and using variables. If a variable value contains spaces, it should be set inside quotes.
+
...
Shell operators such as pipe `|`, background execution `&`, and redirection `>` aren't supported. Including these operators in a command will result in failure to execute the command.
