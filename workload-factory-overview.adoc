...
Analyze costs savings and storage and application benefits when migrating SQL Server databases to Amazon FSx for NetApp ONTAP storage; deploy SQL Servers, databases, and database clones including AWS resources provisioning, storage provisioning, networking, and OS configurations; and continuously optimize SQL Server estates to improve performance, resilience, protection, and cost-efficiency, and to comply with best practices.  
