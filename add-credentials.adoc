...
Learn more about the permissions that are required for each workload capability in the link:permissions-reference.html[Permissions reference].
...
* *Manually*: You create the IAM policy and the IAM role in your AWS account while adding credentials in Workload Factory. AWS GovCloud (US) credentials must be added manually. 
...
+
Workload Factory doesn't support automation in GovCloud accounts.
...
AWS GovCloud(US) credentials must be added manually.
...
* Step 1: Select the services and permissions level that you would like to use and then create IAM policies from the AWS Management Console.
* Step 2: Create an IAM role from the AWS Management Console.
* Step 3: From Workload Factory, enter a name and add the credentials.
...
. For those workload capabilities that offer a choice of permission policies, select the type of permissions that will be available with these credentials.
+
NOTE: If you have a GovCloud account, you don't need the Cost Explorer permissions `ce:GetCostAndUsage` or `ce:GetTags` in the **View, planning, and analysis** policy for Storage. After you copy the script from the code box, remove these permissions and delete the trailing comma from the previous line.
...
. Expand the *Create role* section under the *Permissions configuration* box of the Workload Factory Add Credentials page.

. Copy the JSON document found by clicking on the *JSON code* link in the middle of the *Create role* section highlighted with a red box above.

. Back to the AWS Management Console, select *Roles > Create Role*.

. Under *Trusted entity type*, select *Custom Trust Policy*.

. Replace what is currently in the text box with the JSON document that you copied in step 1.
...
. Back on Add Credentials page in Workload Factory, expand the *Create role* section under *Permission configuration*, and paste the Role ARN in the _Role ARN_ field.
...
NOTE: Adding credentials to an AWS GovCloud (US) account using CloudFormation isn't supported.
