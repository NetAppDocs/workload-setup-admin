...
+
NOTE: GovCloud users are not required to include the following Cost Explorer permissions in the *View, planning, and analysis* policy for Storage: `ce:GetCostAndUsage` and `ce:GetTags`. Remove the permissions after copying the script from the Codebox, and remove the extra comma at the end of the previous row.
