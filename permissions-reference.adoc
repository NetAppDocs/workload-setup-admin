...
VMware has the following permission policies to choose from:

* *Read-only*: detects various resources and tools, and helps to complete relevant wizards. Enables IaC templates to be filled with your specific variables (for example, VPC, security groups, etc.) so that you can execute the IaC directly from your AWS account without providing any modify permissions to Workload Factory.
