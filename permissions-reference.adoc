...
.2+| Get KMS key details for FSx for ONTAP file system encryption
...
| ssm:GetParameter | Deployment and manage operations | Yes | Yes
| ssm:GetParameters | Manage operations | Yes | Yes
| ssm:PutParameter | Deployment and manage operations | Yes | Yes
| ssm:DeleteParameters | Manage operations | Yes | Yes

.9+| Associate network resources to SQL nodes and validation nodes, and add additional secondary IPs to SQL nodes
| ec2:AllocateAddress | Deployment | No | Yes
| ec2:AllocateHosts | Deployment | No | Yes
| ec2:AssignPrivateIpAddresses | Deployment | No | Yes
| ec2:AssociateAddress | Deployment | No | Yes
| ec2:AssociateRouteTable | Deployment | No | Yes
| ec2:AssociateSubnetCidrBlock | Deployment | No | Yes
| ec2:AssociateVpcCidrBlock | Deployment | No | Yes
| ec2:AttachInternetGateway | Deployment | No | Yes
| ec2:AttachNetworkInterface | Deployment | No | Yes
...
| fsx:TagResource | Deployment and Manage operations | No | Yes
...
| ec2:DeleteSecurityGroup | ? | No | Yes
...
| fsx:CreateFileSystem | ? | No | Yes
| fsx:CreateStorageVirtualMachine | ?| No | Yes
| fsx:CreateVolume | ? | No | Yes
...
| kms:CreateGrant | ? | No | Yes
| kms:Describe* | ?| No | Yes
| kms:List* | ? | No | Yes

.7+| Create CloudWatch logs for validation and provisioning scripts running on EC2 instances
| logs:CreateLogGroup | ? | No | Yes
| logs:CreateLogStream | ? | No | Yes
| logs:DescribeLog* | ? | No | Yes
| logs:GetLog* | ? | No | Yes
| logs:ListLogDeliveries | ? | No | Yes
| logs:PutLogEvents | ? | No | Yes
| logs:TagResource| ? | No | Yes

.4+| Create secrets in a user account for the credentials provided for SQL, domain, and FSx for ONTAP
| secretsmanager:CreateSecret | ? | No | Yes
| secretsmanager:GetSecretValue | ?| No | Yes
| secretsmanager:ListSecrets | ? | No | Yes
| servicequotas:ListServiceQuotas | ? | No | Yes

.2+| List customer SNS topics and publish to WLMDB backend SNS as well as customer SNS if selected
| sns:ListTopics | ? | No | Yes
| sns:Publish | ? | No | Yes
...
| ssm:Get* | ? | No | Yes
| ssm:List* | ? | No | Yes
| ssm:PutComplianceItems | ? | No | Yes
| ssm:PutConfigurePackageResult | ? | No | Yes
| ssm:PutInventory | ? | No | Yes
| ssm:SendCommand | ? | No | Yes
| ssm:UpdateAssociationStatus | ? | No | Yes
| ssm:UpdateInstanceAssociationStatus | ? | No | Yes
| ssm:UpdateInstanceInformation | ? | No | Yes
| ssmmessages:* | ? | No | Yes

| Signal CloudFormation stack on success or failure. Permission is restricted to resources starting with WLMDB.
| cloudformation:SignalResource | ? | No | Yes

| Allow role provided for provisioning to use the secrets created programmatically. Permission is restricted to resources starting with WLMDB.
| secretsmanager:PutResourcePolicy | ? | No | Yes

| Tag secrets created by WLMDB deployment. Permission is restricted to resources starting with WLMDB.
| secretsmanager:TagResource | ? | No | Yes
...
| iam:AddRoleToInstanceProfile | ? | No | Yes

| Create instance profile for EC2 and attach the created EC2 role.
| iam:CreateInstanceProfile | ? | No | Yes
