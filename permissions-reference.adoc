...
| To list vpcs on the ai-engine deployment wizard
| cloudformation:DescribeStacks
| Deployment
| Operate
...
| To list FSx file systems on create knowledgeBase wizard
| fsx:DescribeVolumes
| Knowledge base creation
| Operate

| To list FSx file system volumes on create knowledgeBase wizard
| fsx:DescribeFileSystems
| Knowledge base creation
| Operate

| To manage KB on the ai-engine during rebuild operation
|
|
|

| To list FSx file system storage virtual machines on create knowledgeBase wizard
|
|
|

| To use for rebuild operation (move the kb to a new instance) 
|
|
|

| To manage KB on the ai-engine during rebuild
|
|
|

.2+| To use ssm parameter store to save secrets (ECR token, CIFS credentials, tenancy service accounts keys) in a secure way 
| ssm:GetParameter | Deployment | Operate
| ssm:PutParameter | Deployment | Operate

| List log cloudwatch groups- to check if we need to create log group or is it already exists (on deploy and rebuild operations)
|
|
|

.2+|To send the ai-engine logs to cloud watch log group (on deploy and rebuild operations)
| logs:CreateLogGroup | Deployment | Operate
| logs:PutRetentionPolicy | Deployment | Operate

| To send the ai-engine logs to cloud watch log group
| logs:TagResource
| Troubleshooting
| Operate

| To  get ssm response from cloudwatch (when the response is too long)
|
|
|

| To get the ssm response from cloudwatch
|
|
|

.3+| To create cloudwatch log group for bedrock logs during the stack reation (on deploy and rebuild operations)
| logs:CreateLogGroup | Deployment | Operate
| logs:PutRetentionPolicy | Deployment | Operate
| logs:TagResource | Deployment | Operate
