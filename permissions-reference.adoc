...
=== 30 March 2025
You have the ability to manage the `iam:SimulatePrincipalPolicy` permission from the workload factory console when you add additional AWS account credentials or add a new workload capability such as the GenAI workload. Workload factory provides a permissions check by adding the `iam:SimulatePrincipalPolicy` permission to all workload permission policies. The permission simulates workload operations and checks if you have the required AWS account permissions before deploying resources from workload factory. 
