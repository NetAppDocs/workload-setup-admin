...
= Manage service accounts in NetApp Workload Factory
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
. Optionally, you can get an access token for Auth0 management API by executing a client credentials exchange. Refer to link:https://docs.netapp.com/us-en/console-automation/platform/grant_types.html#refresh-token[NetApp Console API documentation^] for information on generating a refresh token.
+
The curl example shows how can you take the client ID and secret and use an API to generate the access token which are time-limited. The token provides several hours of access to the NetApp Workload Factory APIs.
+
[source,JSON]
----
{
...
----
+
Post the output to \https://netapp-cloud-account.auth0.com/oauth/token to get the access token.
