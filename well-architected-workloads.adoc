...
* *ONTAP configuration*
** Recommendation for volumes with the following settings: 
...
The recommendation is to optimize storage efficiency and cost-effectiveness, configure thin provisioning, autosize, and space management options for your FSx ONTAP volumes and LUNs. If not configured properly:
...
Recommendation: For optimal database performance and cost efficiency, Workload Factory recommends moving only snapshots to the capacity tier. This strategy ensures high performance while reducing costs. It is especially recommended to tier snapshots that are older than 7 days. 
