...
* *Swap space*
+
Workload Factory recommends setting the proper swap size to ensure the system can handle memory pressure gracefully, avoiding potential performance degradation or system crashes.
+
Swap space should be sized relatively to RAM:
+
...
* *ASM archive log disk group LUNs* (ASM only)
Multiple LUNs laid out within an Amazon FSx ONTAP volume provides better performance.
...
* *ONTAP configuration*
+
The following configurations should be configured as recommended for optimal performance and cost efficiency of Oracle databases on FSx for ONTAP.
...
* *Operating system*
+
The following configurations should be configured as recommended for optimal operating system performance specifically for Oracle databases.
+ 
** *NFS protocol*
...
*** ASMLib logical block size alignment
