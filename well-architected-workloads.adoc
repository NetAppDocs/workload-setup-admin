...
Workload Factory simplifies the difficult task of manually applying best practices to large storage or database environments; it streamlines this process by combining analysis and remediation of all relevant configurations from the Workload Factory console. The well-architected analysis eliminates the risk of human error and ensures uniformity in storage and database management. By leveraging automation, you can guarantee that the configurations are applied correctly every time and preserved this way over time, thus maintaining the high standards of performance and reliability across your entire storage infrastructure.
...
Workload Factory assesses Microsoft SQL Server and Oracle database configurations and settings related to storage sizing, storage layout, storage configuration, compute, application (SQL Server), and resiliency. The recommendations are based on a combination of best practices for database storage running on Amazon FSx for NetApp ONTAP, and for Microsoft SQL Server database environments, or Oracle database environments. 
...
* *Schedule FSx for ONTAP backups* (applies to Storage and Databases)
+
...
* *Schedule local snapshots* (applies to Storage and Databases)
+
...
* *Cross-region replication* (applies to Databases)
+
...
* *Set up data replication* (applies to Storage)
+
...
* *Increase SSD capacity threshold* (applies to Storage)
+
...
== Best practices and recommendations for well-architected database workloads
Workload Factory provides a set of best practices and recommendations for operating well-architected database workloads. 
...
.7+| To optimize storage efficiency and cost-effectiveness, configure thin provisioning, autosize and space management options for your FSx ONTAP volumes and LUNs.
