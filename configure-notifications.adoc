...
[.lead]
You can configure the NetApp Workload Factory notification service to send notifications as alerts in NetApp Console, to any email addresses associated with the account, or to an Amazon SNS topic. Notifications sent as alerts appear in NetApp Console when you have a Console agent or link deployed. When Workload Factory publishes notifications to an Amazon SNS topic, subscribers to the topic (such as people or other applications) receive the notifications at the endpoints configured for the topic (such as email or SMS messages).

== Configure Workload Factory notifications
You can configure Workload Factory notifications using NetApp Console or the Workload Factory console. 

Options for receiving notifications include:

=== NetApp Console
* *Notifications*: Configure Workload Factory to send notifications that appear under the bell icon in NetApp Console. 
* *Email*: Enable email notifications in NetApp Console to receive notifications to any email addresses associated with the account without requiring Amazon SNS configuration.
* *Amazon SNS*: Configure Workload Factory to send notifications to an Amazon SNS topic.

=== Workload Factory console
* *Email*: Enable email notifications in the Workload Factory console to receive notifications to any email addresses associated with the account without requiring Amazon SNS configuration.
* *Amazon SNS*: Configure Workload Factory to send notifications to an Amazon SNS topic using the Workload Factory console.

IMPORTANT: When you configure notifications in NetApp Console, it takes ownership of notification delivery and the Workload Factory console notification options are disabled and displayed in read-only mode. A tooltip states that notifications are managed from NetApp Console. You can then manage notifications only from NetApp Console.

.Before you begin

The following prerequisites apply only when configuring Amazon SNS notifications:
...
. On the Notifications setup page, enable one or more of the following notification types:
.. *NetApp Console notifications*: Select *Enable NetApp Console notifications* to configure Workload Factory to send notifications in NetApp Console.
.. *Email notifications*: Select *Enable email notifications* to configure Workload Factory to send notifications to any email addresses associated with the account.
.. *Amazon SNS notifications*: Select *Enable SNS notifications*, then follow the instructions to configure Amazon SNS from the Amazon SNS console.
+
...
. Enable one or more of the following notification types:
+
* *Email notifications*: Select *Enable email notifications*. Any email addresses associated with the account are displayed in read-only mode.
* *Amazon SNS notifications*: Select *Enable SNS notifications*, then follow the instructions to configure Amazon SNS from the Amazon SNS console.
+
After you verify the configuration by sending a test notification, select *Apply*.
--
...
You can reduce unnecessary notification traffic and target specific notification types for specific users by applying filters to the notifications. You can do this using an Amazon SNS policy for SNS notifications, and using the notifications settings in NetApp Console.
...
You can use NetApp Console notifications settings to filter notifications that you receive in the Console by severity level, such as Critical, Info, or Warning.
...
| Automatic capacity management event for FSx for ONTAP
| The SSD performance tier for the FSx for ONTAP file system reached the warning threshold capacity/percentage total.
| Capacity management
| Warning
| General storage
| FSx for ONTAP file systems

| Automatic inodes management event for FSx for ONTAP
| The inode count for the FSx for ONTAP volume reached the warning threshold count/percentage total.
| Capacity management
| Warning
| General storage
| FSx for ONTAP file systems

| Suspicious volume activity detected in FSx for ONTAP file system
| NetApp Autonomous Ransomware Protection with AI (ARP/AI) detected suspicious volume activity.
| Ransomware protection
| Critical
| General storage
| FSx for ONTAP file systems

| Latency threshold breach detected
| One or more volumes in the FSx for ONTAP cluster exceeded the latency threshold.
...
| Monthly cost report
| Monthly cost summary for your account, showing spending trends, unusual changes, and the best savings opportunities for FSx for ONTAP file systems.
| Cost report
| Recommendation
| General storage
| FSx for ONTAP file systems
...
| Replication delay
| Replication is taking longer than expected.
| Replication
| Critical
| General storage
| FSx for ONTAP file systems
| Custom dashboard creation succeeded
| Your custom dashboard has been successfully created and is ready to view.
| Custom Dashboards
| Success
| General storage
| Workload Factory account

| Custom dashboard creation failed
| Workload Factory was unable to create your custom dashboard. Try creating the dashboard again.
| Custom Dashboards
| Error
| General storage
| Workload Factory account
