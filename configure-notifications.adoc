...
[.lead]
You can configure workload factory to send notifications to BlueXP or an Amazon SNS topic when certain types of events happen. Notifications sent to BlueXP appear in the BlueXP alerts panel. When workload factory publishes notifications to an Amazon SNS topic, subscribers to the topic (such as people or other applications) receive the notifications at the endpoints configured for the topic (such as email or SMS messages).
